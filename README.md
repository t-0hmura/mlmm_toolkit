...
| Subcommand | Role | Documentation |
|---|---|---|
| `mm-parm` | Generate parm7/rst7 topology via AmberTools | [docs/mm-parm.md](docs/mm-parm.md) |
| `extract` | Extract active-site pocket (cluster model) | [docs/extract.md](docs/extract.md) |
| `define-layer` | Assign 3-layer B-factor encoding | [docs/define-layer.md](docs/define-layer.md) |
| `add-elem-info` | Add/repair PDB element columns (77-78) | [docs/add-elem-info.md](docs/add-elem-info.md) |
| `fix-altloc` | Resolve alternate conformations | [docs/fix-altloc.md](docs/fix-altloc.md) |
...
| `path-opt` | MEP optimization via GSM or DMF | [docs/path-opt.md](docs/path-opt.md) |
| `path-search` | Recursive MEP search with refinement | [docs/path-search.md](docs/path-search.md) |
...
| `oniom-export` | Generate Gaussian ONIOM / ORCA QM/MM input | [docs/oniom-export.md](docs/oniom-export.md) |
| `oniom-import` | Import ONIOM input and reconstruct XYZ/layered PDB | [docs/oniom-import.md](docs/oniom-import.md) |
...
Use [`mlmm define-layer`](docs/define-layer.md) to assign layers automatically based on a model PDB (the ML region).
...
See [Python API Reference](docs/python-api.md) for full documentation.
...
`mlmm-toolkit` ships AI-agent instructions under [`.claude/skills/`](.claude/skills/) so your agent can drive ML/MM ONIOM mechanism studies via Claude Code, Cursor, etc.

The skill bundle covers:
- Design overview (3-layer ML/movable-MM/frozen ONIOM, analytical `hessian_ff`, microiteration)
- CLI subcommands (`extract`, `mm-parm`, `tsopt`, `freq`, `irc`, `dft`, …) with canonical recipes
- Structure I/O (PDB B-factor layer encoding, XYZ / GJF / Amber parm7+rst7, charge & multiplicity)
- Backend installation (UMA / Orb / MACE / AIMNet2 / AmberTools / DFT / xTB) and CUDA/PyTorch quirks
- Workflows & output parsing (`summary.json`, R/TS/P/IM canonical paths, energy diagrams)
- HPC operation (PBS / SLURM templates, dynamic dispatch with `flock` + `pbsdsh`)
