...
Quantum mechanics/molecular mechanics (QM/MM) methods have long been used to analyze enzymatic reaction mechanisms *in silico*. While treating the active site with QM and the remainder with MM yields atomic-level insight, the high computational cost of the QM layer is a major limitation. Replacing the QM region with machine-learning (ML) interatomic potentials yields ML/MM approaches that retain accuracy while greatly reducing cost.

Here, we present **ML/MM tools**, an open-source command-line toolkit centered on an ML/MM calculator that employs AIMNet2 or UMA potentials. The toolkit streamlines the workflow: starting from an enzyme–substrate complex, it produces the reaction free energy ($\Delta G$) and activation free energy ($\Delta G^{\ddagger}$). Because the calculator implements link-atom boundary treatment, amino-acid residues may be included in the ML region, and full-system Hessians can be generated for transition-state (TS) searches and vibrational analyses. To accelerate TS searches in systems of up to around 10,000 atoms, we also integrate a *Partial-Hessian Dimer (PH-Dimer)* approach that updates only the active-site Hessian and suppresses spurious imaginary modes via a mass-scaled flattening loop. Collectively, ML/MM tools make enzymatic mechanistic investigation markedly more accessible.
...
core = MLMMCore(
    ...
    uma_taskname   = "omol"            # See document of fairchem. Default is 'omol', and and it is generally best to leave it unchanged.
