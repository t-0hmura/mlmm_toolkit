...
3. **(Optional) Pre-select the ML region.**
   `mlmm all` extracts the ML region from `-c/--center` and `-r/--radius` automatically. To inspect or hand-edit it first, use [`mlmm extract`](docs/extract.md):
