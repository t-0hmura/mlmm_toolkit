...
def get_data_model(geometry, is_cos, max_cycles):
    ...
    # These shapes are retained as metadata only; optimizer history itself is
    # kept in growable Python lists.  An unlimited run therefore needs no
    # artificial integer cycle cap here.
...
class Optimizer(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        out_dir
            ...
            dumped.
        h5_fn
            Retained for signature compatibility. No HDF5 file is written;
            progress is dumped as trajectories and restart information.
        h5_group_name
            Retained for signature compatibility, and used only to keep the
            per-optimization label unique.
        """
    ...
    def run(self):
        ...
        for self.cur_cycle in cycle_iter:
            ...
            # ``itertools.count`` never exhausts.  This branch is reached only
            # for an explicitly finite cycle budget.
