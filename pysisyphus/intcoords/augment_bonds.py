...
def augment_bonds(geom, root=0, proj=False):
    ...
    if missing_bonds:
        ...
        # Copy the geometry, so frozen atoms, typed constraints, isotopes and the
        # remaining coordinate options are retained. Only the missing auxiliary
        # bonds are appended to the coordinate definition, while 'typed_prims'
        # stays with the copy path that reproduces the current coordinates.
        coord_kwargs = dict(getattr(geom, "coord_kwargs", None) or {})
