...
GPU クラスごとに*どちらの*精度を選ぶか — HPC データセンター GPU（H100 / H200 / A100）では決定論的な計算向けの低ノイズな結果のため `fp64`、fp64 が著しく遅いコンシューマーカードではデフォルトの `fp32` — については [デバイス設定 & HPC セットアップ](device-hpc.md) の「バックエンドごとの精度既定値」を参照してください。
