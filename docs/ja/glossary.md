...
| 用語 | B-factor | 説明 |
|------|----------|------|
| **ML 領域（Layer 1）** | 10.0 | UMA で完全計算（エネルギー・力・ヘシアン）される活性部位原子。 |
| **Hessian-MM（Layer 2）** | 20.0 | ML 領域近傍の MM 原子。hessian_ff でエネルギー・力・ヘシアンを計算。 |
| **Movable-MM（Layer 3）** | 30.0 | ML 領域から離れた MM 原子。hessian_ff でエネルギー・力のみ計算（ヘシアン対象外）。 |
| **Frozen（Layer 4）** | 40.0 | 座標固定の MM 原子。計算不参加。 |
...
| **B-factor** | Temperature Factor | PDB の温度因子カラム。mlmm_toolkit では 4 層の層割り当てをエンコードするために使用。 |
...
| **B-factor 層エンコーディング** | PDB の B-factor カラムを使用して 4 層の層割り当て（10.0, 20.0, 30.0, 40.0）をエンコードする方式。 |
...
- [概念とワークフロー](concepts.md) -- ML/MM 4層システム、ONIOM 分解、ワークフローの全体像
