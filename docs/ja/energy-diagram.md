...
数値エネルギーだけを入力として状態エネルギーダイアグラムを描画します（構造ファイル不要、ML/MM 計算なし）。既にエネルギー値が手元にあり、構造ベースの ML/MM 計算が不要なときに使います。`mlmm energy-diagram` は与えられた数値を可視化するだけで、PDB/XYZ 構造を読み込まず、熱化学（`--thermo`）や DFT（`--dft`）の処理も実行しません。
...
2. 全値を float として解釈し、2 点未満の場合は即座にエラーを返します。
...
result.json   # status / n_points / files と共通 envelope を持つ任意の正規サイドカー
summary.json  # --out-json 成功時に byte 単位で同一の互換ミラー
```
...
| `--out-json / --no-out-json` | 出力画像の隣に正規 `result.json` と同一内容の `summary.json` ミラーを書き出します。 | `--no-out-json` |
