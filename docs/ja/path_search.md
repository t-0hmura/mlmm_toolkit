...
| `--model-pdb PATH` | ML/MM の ML（高レベル）領域原子を定義する PDB。 | 必須 |
...
| `--pre-opt {True\|False}` | セグメンテーション前に端点を LBFGS で事前最適化。 | `True` |
...
| `--args-yaml FILE` | YAML 上書き（`geom`、`calc`/`mlmm`、`gs`、`opt`、`lbfgs`、`bond`、`search`）。 | _None_ |
...
設定の優先順位は **CLI > YAML > デフォルト** です。YAML ルートはマッピングでなければなりません。受け付けるセクション:
...
- ML/MM 物理: 常に `--real-parm7`、`--model-pdb`、`-q/--charge`、任意で `-m/--multiplicity` を指定してください。
...
- `--align True` での最終マージ規則: `--ref-pdb` が提供された場合、最初の参照 PDB がすべてのペアに使用されます。
