...
> **概要:** エンドツーエンドの酵素反応ワークフロー -- ポケット抽出、MM トポロジー準備、任意の段階的スキャン、再帰的 MEP 探索（GSM）、全系へのマージ、任意の TS 最適化、擬似 IRC、熱化学、DFT、DFT//UMA ダイアグラム。
...
- **単一構造 + 段階的スキャン** -- 1 つの PDB と `--scan-lists` を提供します。スキャンで中間体/生成物候補を生成し、MEP の端点として使用します。
  ...
  - 複数ステージは 1 つの `--scan-lists` フラグの後に複数の値として渡します（フラグ自体の繰り返しは不可）。
- **TSOPT のみ** -- 1 つの PDB を提供し、`--scan-lists` を省略して `--tsopt` を設定します。ポケットで TS 最適化を実行し、擬似 IRC、両端の極小化、エネルギーダイアグラムの構築を行います。

```{important}
`--tsopt` は **TS 候補**を生成します。`all` は検証のために IRC と freq を自動実行しますが、機構解釈の前に必ず結果（虚数モード + 端点の結合性）を確認してください。
```

### 概要
- **用途:** ポケット抽出から MEP 探索、任意の TS/freq/DFT 後処理まで、ML/MM による完全なエンドツーエンドの酵素反応研究を行いたい場合。
- **手法:** ポケット抽出 + MM トポロジー（AmberTools）+ GSM MEP 探索 + 任意の TSOPT/IRC/freq/DFT。
- **出力:** `summary.log`、`summary.yaml`、MEP 軌跡、エネルギーダイアグラム、セグメントごとの後処理。
- **デフォルト:** `--opt-mode light`、`--thresh gau`、`--max-cycles 300`、`--preopt` 有効、`--climb` 有効。
...
3. **任意の段階的スキャン（単一入力のみ）**
   - 完全入力 PDB が 1 つのみで `--scan-lists` が指定された場合、抽出されたポケット PDB に対して ML/MM 計算機を使用した段階的な結合距離駆動スキャンを実行します。
...
4. **ポケット入力での MEP 探索（再帰的 GSM）**
   ...
   - `--no-refine-path` で再帰的精密化なしの単一パス `path-opt` GSM チェーンに切り替え可能。
...
5. **全系へのマージと任意の後処理**
   ...
   - `--tsopt`: HEI ポケットで TS を最適化し、EulerPC IRC を実行し、セグメントエネルギーダイアグラムを描画します。
   ...
   - 共有の上書き: `--opt-mode`、`--opt-mode-post`（TSOPT と IRC 後 endpoint-opt のモード上書き）、`--flatten/--no-flatten`、`--micro-step/--no-micro-step`、`--hessian-calc-mode`、`--tsopt-max-cycles`、`--tsopt-out-dir`、`--freq-*`、`--dft-*`。
   - VRAM に余裕がある場合は `--hessian-calc-mode` を `Analytical` に設定することを強く推奨します。

6. **TSOPT のみモード**（単一入力、`--tsopt`、`--scan-lists` なし）
   - ステップ (4)-(5) をスキップし、ポケットで `tsopt` を実行し、擬似 IRC と両端の極小化を行い、R-TS-P の ML/MM エネルギーダイアグラムを構築し、任意で Gibbs、DFT、DFT//UMA ダイアグラムを追加します。
...
| `--freeze-links/--no-freeze-links` | ポケット PDB でリンク親原子を凍結。 | `True` |
...
| `--opt-mode-post [light\|heavy\|hybrid]` | TSOPT/IRC 後端点最適化向けのプリセット上書き。 | _None_ |
...
| `--refine-path/--no-refine-path` | True の場合は再帰的 `path-search`、False の場合は再帰的精密化なしの `path-opt` セグメントチェーン。 | `True` |
| `--hessian-calc-mode CHOICE` | ML/MM ヘシアンモード（`Analytical` または `FiniteDifference`）。 | _デフォルト_ |

TSOPT の最適化モード選択順: `--opt-mode-post`（設定時）-> `--opt-mode`（明示指定時のみ）-> TSOPT デフォルト（`heavy`）。  
`--opt-mode-post hybrid` を指定した場合、TSOPT 側は `heavy` にフォールバックし、IRC 後 endpoint-opt は `opt --opt-mode hybrid` で実行されます。
...
| `--flatten/--no-flatten` | `tsopt` での余分な虚数モードフラットニングを有効化。 | `False` |
| `--micro-step/--no-micro-step` | `tsopt` に転送。TSOPT が heavy モードで実行される場合、`--no-micro-step` は `rsirfo.max_micro_cycles=1` を強制。 | `True` |
...
- 抽出半径: `--radius` または `--radius-het2het` に `0` を渡すと、抽出器により内部的に `0.001 A` にクランプされます。
