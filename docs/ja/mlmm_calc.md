...
実装では任意に MM 原子をレイヤーに分割し、どの原子が MM 有限差分ヘシアンを受け取り、どの原子が凍結されるかを制御できます:

- **ML 領域**（B 因子 = 10.0）: UMA 機械学習ポテンシャルで処理
- **Hessian-MM**（B 因子 = 20.0）: ヘシアン計算に含まれる MM 原子
- **Movable-MM**（B 因子 = 30.0）: 最適化中に移動する MM 原子
- **Frozen**（B 因子 = 40.0）: 固定された MM 原子
