...
PDB ファイルから代替位置（altLoc）指示子を削除し、各原子について占有率に基づいて最良のコンフォーマーを選択し、重複を除去します。altLoc カラム（列 17、1-based）はスペース 1 文字に置換され（1 文字の置換のみで、フォーマットのシフトや再構成は行いません）、同じ原子が複数の altLoc 状態で出現する場合は占有率が最も高いコピーを保持します（同点の場合、または占有率が欠損している場合はファイル内で最初に出現したもの）。`ATOM` / `HETATM` レコードは altLoc の選択とブランク化の対象となり、`ANISOU` レコードは対応する ATOM/HETATM 行（同じシリアル番号）が保持される場合のみ保持されます。

## 使いどころ

- 下流の ML/MM 準備の前に、altLoc 文字を含む PDB をクリーンアップする。
- まず [add-elem-info](add-elem-info.md) で元素カラムを修復してから altLoc を解決する。
- altLoc 文字を含まないファイルは `--force` を設定しない限りスキップされる。
...
| 入力 | 必須 | 説明 |
| --- | --- | --- |
| `-i, --input PATH` | はい | 入力 PDB ファイルまたはディレクトリ |
| `-o, --out PATH` | いいえ | 出力ファイル（入力がファイルの場合）またはディレクトリ（入力がディレクトリの場合）。デフォルトは `<input>_clean.pdb`（ファイル）または `<input>_clean/`（ディレクトリ） |
