...
OMol で訓練された UMA をデフォルトの fp32 から fp64 に切り替えると、TSopt + Hessian に対して
...
方向ごとの FD-Hessian ループは、この複合的なフットプリントに収まるようにサイズ設定されています。**方向ごとのループを
バッチ化テンソルにリファクタリングしないでください**（GPU smoke ゲート全体（`tests/smoke/run.sh`）の再実行と
ピーク VRAM の監視を伴わない限り）。さもなければ全タンパク質 ML/MM の `all` ジョブが OOM します。ステージランナーは
