...
- **VRAM 不足**: ML 領域サイズを縮小、ヘシアン対象 MM 領域を縮小、ノード数を削減（`--max-nodes`）、または軽量なオプティマイザ設定（`--opt-mode grad`）を使用。エラーメッセージに "reserved but unallocated" が含まれる場合は `export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` を設定して VRAM フラグメンテーションを軽減する
