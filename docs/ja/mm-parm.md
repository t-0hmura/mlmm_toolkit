...
- **仮想サイトを持つ 4 点モデルの水（OPC, TIP4P/-Ew, TIP5P）は、mlmm-toolkit のデフォルト MM バックエンドではまだ対応していません** -- 質量ゼロの追加点（Amber の `EPW`、元素 `EP`）が自由原子として扱われ、構造最適化が収束しません。**3 点モデルの水**を使用してください: デフォルトの `ff19SB` セットは推奨される 3 点モデルの **OPC3** を、`--ff-set ff14SB` は TIP3P を構築します。4 点モデルの水を保持する必要がある場合は `--mm-backend openmm` で実行すると仮想サイトが正しく配置されますが、有限差分 MM Hessian のため低速です。
