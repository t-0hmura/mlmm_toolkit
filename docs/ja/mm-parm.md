...
`mlmm mm-parm` は PDB から Amber トポロジー/座標ファイル（parm7/rst7/pdb）を生成します。不明な残基は GAFF2（AM1-BCC 電荷）で自動的にパラメータ化され、ジスルフィド結合検出や PDBFixer による任意の水素付加にも対応します。パイプライン全体は「処理の流れ」、力場や水素付加のフラグは「CLI オプション」を参照してください。金属酵素、糖鎖、非標準アミノ酸・翻訳後修飾、MD スナップショット入力には不向きで、これらは外部で用意したトポロジーを `--parm` で指定します（[注記](#注記)を参照）。
...
- **仮想サイトを持つ 4 点モデルの水（OPC, TIP4P/-Ew, TIP5P）は、mlmm-toolkit の既定 MM バックエンドでは _まだ_ 未対応です** -- 質量ゼロの追加点（Amber の `EPW`、元素 `EP`）が自由原子として扱われ、構造最適化が収束しません。**3 点モデルの水**を使用してください: デフォルトの `ff19SB` セットは推奨される 3 点モデルの **OPC3** を、`--ff-set ff14SB` は TIP3P を構築します。4 点モデルの水を保持する必要がある場合は `--mm-backend openmm` で実行すると仮想サイトが正しく配置されますが、有限差分 MM ヘシアンのため低速です。
