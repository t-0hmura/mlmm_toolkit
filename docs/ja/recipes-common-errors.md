...
症状は分かっているものの、どのサブコマンドのページから見ればよいか迷うときに使います。
...
| **収束** | | |
| TSOPT/IRC が収束しない | ステップ長を縮小（RFO 系 TS オプティマイザでは trust_radius、Dimer/L-BFGS では max_step）、サイクル数を増やし、まず TS の品質を検証 | [計算 / 収束](troubleshooting.md#計算--収束の問題) |
| エネルギーが平坦なのに最適化が停滞（MLIP のノイズフロアの可能性） | `--max-cycles` に任せるか、`--stop-plateau` で早期停止を opt-in する。判定が早すぎ/遅すぎる場合は `--stop-plateau-thresh` / `--stop-plateau-window` を調整 | [プラトー・フォールバック](troubleshooting.md#最適化が停滞するがエネルギーはもう変わっていないmlip-の力ノイズフロア) |
| **プロット** | | |
...
- TS 候補の虚振動モードがちょうど 1 つか、その変位と IRC の接続性を確認します。
- ステップ長（trust_radius / max_step）を縮小し、サイクル上限を増やす。RFO 系 TS オプティマイザの `trust_max` デフォルトは 0.10 bohr です。
- エネルギーの平坦化だけでは原因を特定できません。MLIP の力ノイズフロアも候補の一つです。プラトー停止はデフォルトで無効で、`--stop-plateau` を指定すると `stalled`（未収束）として停止します（[トラブルシューティング](troubleshooting.md#最適化が停滞するがエネルギーはもう変わっていないmlip-の力ノイズフロア) を参照）。
