...
| `--detect-layer / --no-detect-layer` | B 因子レイヤー（B=0/10/20）を読み取ります。ML 原子集合を明示した場合は MM 側のレイヤーだけを保持し、明示しない場合は B 因子が ML 原子集合も定義します。無効時は `--model-pdb` または `--model-indices` が必要です。 | `True` |
