...
- **出力:** `final_geometries.trj`（経路）および `gsm_hei.xyz`（HEI）、任意で `.pdb` コンパニオン。
- **デフォルト:** `--climb True`、`--max-nodes 10`、`--max-cycles 300`。
...
設定は YAML（`geom`、`calc`/`mlmm`、`gs`、`opt`）から取得され、優先順位は **CLI > YAML > デフォルト** です。CLI は常に ML/MM 入力ファイル、電荷、多重度を上書きします。`StringOptimizer.align` は無効のままです。代わりに、外部の Kabsch ベースのアライメント/精密化ルーチンが、経路成長前にすべてのイメージをアラインします（YAML/CLI からの凍結原子を尊重）。
...
3. **任意の事前最適化** -- `--preopt True` の場合、各端点はアライメントとストリング成長の前に LBFGS（同じ ML/MM 計算機を使用）で事前最適化されます。LBFGS サイクル数は `--preopt-max-cycles`（デフォルト: 10000）で制御されます。
4. **ストリング成長** -- PySisyphus `GrowingString` が端点を含む `(max_nodes + 2)` イメージを使用して端点間の経路を成長させます。
5. **クライミングイメージ** -- `--climb True` の場合、ストリングが完全に成長した後にクライミングイメージ精密化が適用され、最高エネルギーイメージ（HEI）が報告されます。
...
| `--climb {True\|False}` | ストリング完全成長後のクライミングイメージ精密化を有効化。 | `True` |
| `--preopt {True\|False}` | アライメント/ストリング成長前に各端点を LBFGS で事前最適化。 | `False` |
| `--preopt-max-cycles INT` | 端点事前最適化サイクルの上限。 | `10000` |
| `--thresh TEXT` | 収束プリセット上書き（`gau_loose`、`gau`、`gau_tight`、`gau_vtight`、`baker`、`never`）。 | `gau` |
| `--dump {True\|False}` | `out_dir` 内にオプティマイザー軌跡とリスタートをダンプ。 | `False` |
| `--out-dir TEXT` | 出力ディレクトリ。 | `./result_path_opt/` |
| `--args-yaml FILE` | YAML 上書き（セクション `geom`、`calc`/`mlmm`、`gs`、`opt`）。 | _None_ |
...
├─ gsm_hei.xyz                 # 最高エネルギーイメージ（XYZ、常に書き出し）
├─ gsm_hei.pdb                 # PDB 形式の HEI（参照 PDB が利用可能な場合）
├─ align_refine/               # 外部アライメント/精密化の成果物
├─ preopt/                     # 端点事前最適化出力（--preopt True 時）
└─ <optimizer dumps>           # --dump True または opt.dump_restart > 0 の場合
```

## YAML 設定（`--args-yaml`）

YAML パラメータは内部デフォルトを上書きし、CLI が YAML を上書きします。
