...
すべてのブール値 CLI フラグは **4 つの記法**を受け付けます。
...
PDB 入力の場合、`--ligand-charge` で非標準残基（基質、補因子、金属イオン）の電荷のみを指定します。選択した ML 領域/model system の正味電荷は、標準アミノ酸の電荷、イオン電荷、指定したリガンド電荷を合計して**自動算出**されるため、ML 領域内の原子を手動で数える必要がありません。
