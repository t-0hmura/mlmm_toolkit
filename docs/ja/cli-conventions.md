...
すべてのブール値 CLI フラグは **4 つの記法を恒久的に**受け付けます（廃止予定なし）。
...
4 記法はすべて root CLI の `bool_compat` シンセサイザを経由します。`tests/test_bool_compat_cli.py` がリリースごとに登録済みの全ブールオプション × 全記法を走査するため、登録漏れは CI で検出されます。
...
`scan` / `scan2d` / `scan3d` と計算系サブコマンド（`opt` / `path-opt` / `path-search` / `tsopt` / `freq` / `irc` / `dft`）に加え、ユーティリティ系（`mm-parm` / `define-layer` / `add-elem-info` / `trj2fig` / `energy-diagram` / `oniom-export`）も同様に `--help` は主要オプションのみ、`--help-advanced` で全オプションを表示します。`extract` と `fix-altloc` も段階的 help に対応し、`--help-advanced` で parser の全オプションを示します。
...
4. `.gjf` テンプレートのメタデータ（Gaussian 形式の電荷/スピンヘッダ。入力が `.gjf` のとき `opt`、`tsopt`、`freq`、`irc`、`scan`、`dft`、`oniom-export` に適用）。
5. デフォルト: なし（未解決なら中断）。
...
| `path-opt` / `path-search` | `grad`（`lbfgs`） | —（`hess` は未配線） | `grad` | GSM / DMF ノードの内部ストリングオプティマイザ。 |
