...
- **MLIP バックエンド**: ML 領域のエネルギー・力・ヘシアン計算。デフォルトは UMA（fairchem-core）。ORB（`pip install "mlmm-toolkit[orb]"`）、AIMNet2（`pip install "mlmm-toolkit[aimnet]"`）も利用可能。MACE も利用可能ですが、`e3nn` バージョン競合のため `fairchem-core` を先にアンインストールする必要があります（`pip uninstall fairchem-core && pip install mace-torch`）。
...
4. path-search - MEP 探索（単一パス path-opt が既定）; `--refine-path` で再帰 path-search に切替
5. tsopt - 遷移状態最適化
6. freq - 振動解析と熱化学
7. dft - DFT 一点計算
```
...
- 既定で単一パス `path-opt` による **MEP 探索**を実行（GSM per adjacent pair、出力は `path_opt/` 以下）
...
- 既定の `all` ワークフローは連結されたステージに対して単一パス `path-opt` GSM チェーンを実行します。
...
| `--refine-path/--no-refine-path` | 単一パス `path-opt`（既定）vs `--refine-path` で再帰 `path-search` |
