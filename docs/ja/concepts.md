...
B-factor 値は PDB ファイルの温度因子カラム（列 61-66）にエンコードされます。`define-layer` サブコマンドが ML 領域からの距離に基づいて自動設定します（`--radius-freeze`（デフォルト 8.0 Å）以内を Movable-MM、それより遠方を Frozen に割り当て）。Hessian 対象 MM 原子は `hess_cutoff` や `hess_mm_atoms` で別途制御します。
