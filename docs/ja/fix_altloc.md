...
### 概要
- **用途:** PDB に代替コンフォーマー（altLoc A/B/...）が含まれており、下流処理の前に単一コンフォーマーに解決する必要がある場合。
- **手法:** 原子同一性キーごとに最高占有率のコンフォーマーを選択し、altLoc カラム（17）をブランク化。
- **出力:** altLoc カラムがブランク化され重複が除去されたクリーンな PDB ファイル。
- **デフォルト:** altLoc 文字がないファイルはスキップ。出力は `<input>_clean.pdb`。
- **次のステップ:** [extract](extract.md) または [mm-parm](mm_parm.md)。
...
- [all](all.md) -- 自動 altLoc 修正を含むエンドツーエンドワークフロー
