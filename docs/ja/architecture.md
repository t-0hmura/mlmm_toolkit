...
| **L4b Infra (I/O)** | `mlmm/io/` | 出力レイアウト、サマリー、軌跡、PDB 修正、エネルギー図、ヘシアンキャッシュ、解析的ヘシアン glue | `core/` |
| **L5 Foundation** | `mlmm/core/` | defaults (単一の真実源)、utils (PDB / XYZ / プロットヘルパー)、将来の `errors.py` / `types.py` | (none) |
| (bundle, not a layer) | `<repo>/pysisyphus/`、`<repo>/thermoanalysis/`、`<repo>/hessian_ff/` | repo-internal フォーク (オプティマイザ / 熱化学 / 解析的 MM ヘシアン) | (sibling, layer-external) |
...
**L4b `io/`** (7 ファイル)。出力側の I/O 関連: ステージごとのサマリーライター、エネルギー図、軌跡レンダリング、PDB altloc 修正、ヘシアンキャッシュ、数値ヘシアン構築 + 振動数 / 振動 I/O (`hessian_calc.py`)、調和拘束のセットアップ。`io/` は決して `workflows/` に依存しません。出力フォーマットはここで所有され、ステージランナーから消費されます。

**L5 `core/`**。最下層です。`defaults.py` はすべての CLI デフォルトの **単一の真実源** です — 数値を他のどこかに追加する前にここを grep してください。`utils.py` は PDB / XYZ / プロットヘルパーの ~2,560 LOC の寄せ集めで、将来的には `utils/{pdb,plot,coord,yaml,freeze,input_prep}.py` へ分割する可能性があります。`logging.py` (`-v` / `-vv` の配線)、`calc_eval.py` (ステージごとの calc 評価)、`residue_data.py` (残基テーブル) もここにあります。内部専用モジュール `errors.py`、`types.py` / `_stage.py` は、追加された時点でここに導入されます。
...
| concern | file |
|---|---|
| `summary.json` / `summary.md` ライター | `mlmm/io/summary.py` |
| Plotly エネルギー図 | `mlmm/io/energy_diagram.py` |
| Trajectory → PNG / HTML / SVG / PDF | `mlmm/io/trj2fig.py` |
| PDB altloc 解決 | `mlmm/io/pdb_fix.py` |
| インメモリヘシアンキャッシュ (run ごとの TTL) | `mlmm/io/hessian_cache.py` |
| 数値ヘシアン構築 + 振動数 / 振動 I/O | `mlmm/io/hessian_calc.py` |
...
| concern | file |
|---|---|
| **すべての CLI デフォルト (単一の真実源)** | `mlmm/core/defaults.py` |
...
## 6. 内蔵フォーク (repo-internal、2 リポジトリ独立進化)
...
| `hessian_ff/` | **NO — PyPI 404、バンドル必須** | MM 力場上の解析的ヘシアン | `pysisyphus/` と同じ |
...
1. `mlmm/core/defaults.py` — デフォルト値テーブルを内面化する。下流のすべてがここから読み取る。
...
9. `mlmm/workflows/irc.py` — VRAM 衛生 + マクロ / マイクロ IRC。
