...
**L5 `core/`**。最下層です。`defaults.py` はすべての CLI デフォルトの **単一の真実源** です — 数値を他のどこかに追加する前にここを grep してください。`utils.py` は PDB / XYZ / プロットヘルパーの ~3,200 LOC の寄せ集めで、将来的には `utils/{pdb,plot,coord,yaml,freeze,input_prep}.py` へ分割する可能性があります。`logging.py` (`-v` / `-vv` の配線)、`calc_eval.py` (ステージごとの calc 評価)、`residue_data.py` (残基テーブル) もここにあります。内部専用モジュール `errors.py`、`types.py` / `_stage.py` は、追加された時点でここに導入されます。
