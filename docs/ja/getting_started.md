...
- **MLIP バックエンド**: ML 領域のエネルギー・力・ヘシアン計算。デフォルトは UMA（fairchem-core）。ORB（`pip install "mlmm_toolkit[orb] @ git+https://github.com/t-0hmura/mlmm_toolkit.git"`）、AIMNet2（`pip install "mlmm_toolkit[aimnet2] @ git+https://github.com/t-0hmura/mlmm_toolkit.git"`）も利用可能。MACE は e3nn 競合のため別環境が必要
...
pip install "mlmm_toolkit[orb] @ git+https://github.com/t-0hmura/mlmm_toolkit.git"       # ORB バックエンド
pip install "mlmm_toolkit[aimnet2] @ git+https://github.com/t-0hmura/mlmm_toolkit.git"   # AIMNet2 バックエンド
