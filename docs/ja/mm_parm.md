...
- **入力:** 1 つの PDB ファイル（`--add-h` でない限りそのまま使用）。
...
- **要件:** AmberTools（tleap、antechamber、parmchk2）が PATH 上にあること。PDBFixer + OpenMM は `--add-h` の場合のみ必要。
...
`--add-ter`（デフォルト）の場合、`--ligand-charge` に名前が含まれる残基、水残基、またはイオンの連続ブロックの**前後**に TER レコードが挿入されます。連続する同種の残基間には TER レコードは挿入されません。

### 失敗時の動作

ビルドが失敗した場合でも、`--add-h` で水素付加が成功していれば、水素付加済み PDB は LEaP PDB と同じ命名規則（`--out-prefix` 省略時は `<input_stem>_parm.pdb`、指定時は `<out_prefix>.pdb`）でディスクに書き出されます。
