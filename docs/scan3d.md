...
> **Summary:** Perform a three-distance (d1, d2, d3) grid scan with harmonic restraints and ML/MM relaxations. You provide one `--scan-lists` literal with three quadruples `(i, j, low, high)`.

### Quick reference
- **Input:** One full enzyme PDB + **one** `--scan-lists` literal (three quadruples).
...
1. Load the structure through `geom_loader`, resolve charge/spin from CLI, and
   optionally run an unbiased preoptimization when `--preopt True`.
...
| `--scan-lists TEXT` | Single Python literal with three quadruples `(i,j,low,high)`. `i`/`j` can be integer indices or PDB atom selectors. | Required |
...
| `--dump {True\|False}` | Write inner d3 scan TRJs per (d1, d2) slice. | `False` |
