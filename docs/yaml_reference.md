...
This document provides a comprehensive reference for all YAML configuration options used across `mlmm` subcommands. In `mlmm all`, layered YAML is supported via `--config` and `--override-yaml` (`--args-yaml` is a legacy alias of `--override-yaml`) with precedence **defaults < config < CLI < override-yaml**.

---

## Overview

All `mlmm` subcommands that accept `--args-yaml` share common configuration sections:
...
| [`mlmm`](#mlmm-section) | ML/MM calculator configuration (alias: `calc`) | all, opt, scan, scan2d, scan3d, tsopt, freq, irc, path-opt, path-search |
...
ML/MM calculator configuration. The section name `mlmm` is the primary name; `calc` is accepted as an alias for backward compatibility.
...
mlmm:
  ...
  hessian_calc_mode: null                # Alias for ml_hessian_mode (backward compatibility)
...
- The `calc` section name is accepted as an alias for `mlmm` for backward compatibility
...
# Use with: mlmm all -i R.pdb P.pdb -c 'LIG' --args-yaml config.yaml
