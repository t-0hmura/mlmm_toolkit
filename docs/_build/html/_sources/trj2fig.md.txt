...
> **Summary:** Extract energies from XYZ trajectory comment lines, compute relative or absolute energy profiles, and export polished Plotly figures and CSV tables.

### Quick reference
- **Input:** An XYZ trajectory whose second line (comment) stores Hartree energies.
...
## Workflow
1. Parse the XYZ trajectory and extract Hartree energies from the first
 floating-point number found on each frame's comment line. If no energies are
 found or a frame comment lacks a parseable energy, an error is raised.
