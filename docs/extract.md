...
| `--include-H2O {true\|false}` | true | Include water molecules (HOH/WAT/TIP3/SOL) in the pocket. |
| `--exclude-backbone {true\|false}` | true | Exclude backbone atoms from non-substrate amino acids. |
| `--add-linkH {true\|false}` | true | Add link hydrogens (HL in residue LKH) to cap severed bonds. |
...
| `--verbose {true\|false}` | false | Enable verbose logging of residue selection and atom counts. |
...
2. **Standard cutoff** (`--radius`, default 2.6 A):
   - With `--exclude-backbone true` (default): amino-acid residues qualify only if a **non-backbone** atom is within the cutoff.
   - With `--exclude-backbone false`: any atom within the cutoff qualifies the residue.
...
4. **Waters** are included by default (`--include-H2O true`).
...
- With `--exclude-backbone true` (default): delete main-chain atoms on all non-substrate amino acids (with PRO/HYP exceptions).

---

## Link Hydrogens (`--add-linkH`)

When enabled (default), link hydrogens are added at **1.09 A** along the cut-bond vector (carbon-only caps). They are written as **HETATM** records with:
- Atom name: `HL`
- Residue name: `LKH`
- Chain: `L`
...
- [define-layer](define_layer.md) -- Assign 4-layer ML/MM partitioning
