...
# `all`-pipeline layout dirnames — one source for the deliverable/scratch split.
SEGMENTS_DIRNAME = "segments"  # per-segment deliverables: segments/seg_NN/<subcmd>/
WORK_DIRNAME = "_work"  # pipeline-wide scratch (safe to rm -rf)
...
MLMM_CALC_KW: Dict[str, Any] = {
    ...
    "hess_cutoff": None,      # Å, MM atoms within this distance of ML get MM Hessian (None = all movable MM, so freq/irc/opt analyze every movable atom). tsopt's Dimer TS search pins an ML-only Hessian (0.0) only under the opt-in ml_only_hessian_dimer.
