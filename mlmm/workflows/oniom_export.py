...
from mlmm.domain.add_elem_info import guess_element as _guess_element
...
def _get_total_charge(parm) -> int:
    ...
    # systems), so a genuine integer total like -5 routinely appears as
...
def _estimate_link_h_position_fixed(parm, qm_idx: int, mm_idx: int, bond_length: float) -> Optional[np.ndarray]:
    """Fixed-distance rule (legacy): r_H = r_QM + u(QM->MM) * d, d in {1.09, 1.01} Å."""
...
def _estimate_link_h_position_scaled(
    parm, qm_idx: int, mm_idx: int, qm_elem: str, mm_elem: str,
) -> Optional[np.ndarray]:
    """Scaled g-factor rule (MLMMCore default): r_H = r_QM + g * (r_MM - r_QM).

    Matches the Morokuma/Dapprich convention used by mlmm.backends.mlmm_calc
    when ``link_atom_method='scaled'`` (defaults.py:58).
    """
    # Lazy import to avoid pulling torch/openmm into the lightweight exporter path.
    from mlmm.backends.mlmm_calc import _get_g_factor as _gfactor
    qm_xyz = _atom_xyz(parm, qm_idx)
    mm_xyz = _atom_xyz(parm, mm_idx)
    vec = mm_xyz - qm_xyz
    if float(np.linalg.norm(vec)) < 1.0e-12:
        return None
    try:
        g = float(_gfactor(qm_elem, mm_elem, "H"))
    except KeyError:
        return None
    return qm_xyz + vec * g
...
def _build_link_atom_specs(
    ...
    *,
    ...
    Each value contains:
      ...
      - bond_length: QM-H bond length used for placement (fixed-method, informational only for scaled)
      - position: estimated link-H coordinate (Å), when available

    ``link_atom_method='scaled'`` (default, matches MLMMCore runtime) places
    the link H via the Morokuma/Dapprich g-factor.
    ``link_atom_method='fixed'`` uses the legacy 1.09/1.01 Å rule.
    """
    method = str(link_atom_method).lower()
    if method not in {"scaled", "fixed"}:
        raise ValueError(
            f"link_atom_method must be 'scaled' or 'fixed', got '{link_atom_method}'."
        )
    ...
    for qm_idx, mm_idx in _find_qmmm_boundary_pairs(parm, qm_indices):
        ...
        if method == "scaled":
            mm_elem = _normalize_element_symbol(_get_parm_element(parm.atoms[mm_idx]))
            link_pos = _estimate_link_h_position_scaled(
                parm, qm_idx=qm_idx, mm_idx=mm_idx, qm_elem=qm_elem, mm_elem=mm_elem,
            )
        else:
            link_pos = _estimate_link_h_position_fixed(
                parm, qm_idx=qm_idx, mm_idx=mm_idx, bond_length=bond_len,
            )
...
# DO NOT INLINE: each emitted Gaussian directive carries a chemistry decision; the function reads top-to-bottom as the actual format spec. Limitations section (Amber periodicity>4, SCEE/SCNB) is part of the contract.
...
# DO NOT INLINE: (export_gaussian): format-specific writer for Gaussian 16 ONIOM input; splitting would scatter format knowledge. Keep monolithic.
def export_gaussian(
    parm7_path: Path,
...
# DO NOT INLINE: (export_orca): format-specific writer for ORCA 5+ ExtTool input; splitting would scatter format knowledge. Keep monolithic.
def export_orca(
    parm7_path: Path,
    model_pdb: Optional[Path],
    output_path: Path,
    ...
    if orcaff_path is None:
        ...
        # DO NOT INLINE: ORCA MM module needs its own ORCAFF.prms generated by bundled `orca_mm`; production users may not have it. Graceful warning + manual-command echo (`Run manually:` is user-facing rescue) is the correct compromise.
