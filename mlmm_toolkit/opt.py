...
@click.option(
    ...
    type=click.Path(path_type=Path, exists=True, dir_okay=False),
    default=None,
    help="YAML with extra arguments (sections: geom, calc/mlmm, opt, lbfgs, rfo).",
)
def cli(
    input_path: Path,
    ...
    try:
        ...
        calc_cfg["model_charge"] = charge
        calc_cfg["model_mult"] = spin
        calc_cfg["input_pdb"] = str(prepared_input.source_path)  # Use PDB topology (or ref_pdb if provided)
        ...
        if radius_partial_hessian is not None:
            calc_cfg["hess_cutoff"] = radius_partial_hessian
        if radius_freeze is not None:
            calc_cfg["movable_cutoff"] = radius_freeze
            calc_cfg["use_bfactor_layers"] = False
        if radius_partial_hessian is not None or radius_freeze is not None:
            click.echo(
                f"[layer] Applied distance cutoffs: "
                f"hess={radius_partial_hessian} Å, freeze={radius_freeze} Å"
            )
