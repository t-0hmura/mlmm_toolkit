...
def _read_pdb_geometry(pdb_path: Path) -> Tuple[np.ndarray, List[str]]:
    ...
    with pdb_path.open("r") as f:
        for line in f:
            ...
            # Element: prefer columns 77-78; fall back to atom-name alignment rules.
            # Also guard against 1-letter misalignment when atom name encodes a 2-letter element (e.g., MG).
...
def _parse_pdb_atoms_with_meta(pdb_path: Path) -> List[Dict[str, Any]]:
    ...
    with pdb_path.open("r") as f:
        ...
        for line in f:
            ...
            elem_inferred = _infer_element_from_pdb_atom_name(line[12:16])
            elem = elem_field or elem_inferred
...
%mem={mem}
%nprocshared={nproc}
#p opt=(quadmac,calcfc,maxstep=5) oniom({method}:amber=softonly)
scf=(xqc,intrep,maxconventionalcyc=80) geom=connectivity nosymm iop(2/15=3) Amber=(FirstEquiv)

ONIOM inputfile generated by mlmm oniom-gaussian from {parm_path}.
...
@click.command(
    name="oniom-gaussian",
    help="Generate Gaussian ONIOM input from Amber parm7 topology.",
...
@click.option(
    ...
    required=True,
    help="Output Gaussian input file (.com or .gjf).",
)
@click.option(
    "--method",
    type=str,
...
@click.option(
    ...
    default=6.0,
    show_default=True,
    help="Distance cutoff for movable atoms (Angstrom).",
)
@click.option(
    "--nproc",
    type=int,
...
@click.command(
    name="oniom-orca",
    help="Generate ORCA QM/MM input from Amber parm7 topology.",
    context_settings={"help_option_names": ["-h", "--help"]},
)
@click.option(
    "--parm",
...
@click.option(
    ...
    type=click.Path(path_type=Path, dir_okay=False),
    required=True,
    help="Output ORCA input file (.inp).",
)
@click.option(
    "--method",
    type=str,
...
@click.option(
    ...
    type=int,
    default=None,
    help="Total charge of full QM+MM system for Charge_Total. Defaults to topology total charge.",
)
@click.option(
    "--total-mult",
    type=int,
    default=None,
    help="Total multiplicity of full QM+MM system for Mult_Total. Defaults to --multiplicity.",
)
@click.option(
    "--nproc",
    type=int,
...
@click.option(
    ...
    default=6.0,
    show_default=True,
    help="Distance cutoff (Å) used to define ActiveAtoms when no layer B-factors are present.",
)
@click.option(
    "--orcaff",
    type=click.Path(exists=True, path_type=Path),
    default=None,
    help="Path to ORCAFF.prms (QM/MM force field parameters). If omitted, the exporter uses/creates <parm7_stem>.ORCAFF.prms in the output directory.",
)
@click.option(
    "--convert-orcaff/--no-convert-orcaff",
    default=True,
    show_default=True,
    help="If ORCAFF.prms is missing and orca_mm is available, try to generate it automatically.",
)
def cli_orca(
    parm7: Path,
    input_coords: Optional[Path],
