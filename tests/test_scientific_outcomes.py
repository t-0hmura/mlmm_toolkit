...
#    success because a diagram exists; nothing scientifically usable exists, so
#    it must be `failed` while `execution_status` reports the completed run.
# ---------------------------------------------------------------------------
...
def test_nonconverged_bridge_prevents_mixed_path_success() -> None:
    ...
    segments = [
        SegmentReport(tag="seg_01", barrier_kcal=10.0, delta_kcal=0.0,
                      summary="", kind="seg", seg_index=1, converged=True),
        SegmentReport(tag="bridge_01", barrier_kcal=0.0, delta_kcal=0.0,
                      summary="", kind="bridge", seg_index=2, converged=False),
        SegmentReport(tag="seg_02", barrier_kcal=8.0, delta_kcal=-1.0,
                      summary="", kind="seg", seg_index=3, converged=True),
    ]
...
# ---------------------------------------------------------------------------
# 8. The ALL-pipeline aggregate consumes endpoint-owned outcomes
        ...
        post = {
            **base,
            ...
            "endpoint_opt": {
                "reactant_converged": True,
                "product_converged": True,
                "connectivity_validated": verdict,
            },
        }
        truth = _pipeline_aggregate_truth(
            summary,
