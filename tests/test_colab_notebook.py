...
def test_every_distance_spinner_uses_point_two_step(
    ...
    expected = {
        ...
        "scan_max_step_size", "embedcharge_cutoff", "hess_cutoff",
        "movable_cutoff", "amplitude_ang", "step_size", "max_step_size",
        "radius_freeze",
    }
...
def test_colab_compact_selection_upload_viewer_and_advanced_contracts(
    ...
    leaf_html = app["_summary_html"](str(leaf_result))
...
def test_advanced_numeric_controls_use_the_effective_cli_default() -> None:
    ...
    namespace = {
        ...
        "refresh": lambda: None,
        "_flag_row": lambda widget, _help, **_kwargs: types.SimpleNamespace(children=(widget,)),
    }
    exec(compile(ast.Module(body=functions, type_ignores=[]),
                 str(NOTEBOOK), "exec"), namespace)
...
def test_energy_levels_match_cli_width_and_fit_annotations(
    ...
    from mlmm.core.utils import build_energy_diagram
...
def test_results_explicit_pdb_state_preserves_ownership_with_mismatched_xyz(
    monkeypatch, tmp_path: Path,
) -> None:
    app, _ = _execute_app(monkeypatch, tmp_path)
    ...
    # MLMM has no scalar JSON fallback; a matching XYZ supplies its own energy.
    _write_result_xyz(xyz, [((0., 0., 0.), (0.7, 0., 0.))], [-10.])
    app["_load_trajectory"](str(pdb), str(tmp_path))
