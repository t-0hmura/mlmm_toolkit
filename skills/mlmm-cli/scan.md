...
specs (`("CS1 SAM 320", "C7 GPP 321", 1.60)`). Multiple literals after a
single `-s` flag chain stages sequentially: each stage starts from the
previous stage's final geometry.
...
├── stage_NN/                  # per-stage relaxed snapshots
│   ├── scan_trj.xyz
│   ├── scan.pdb               # when input is PDB or --ref-pdb supplied
│   └── result.{xyz,pdb}       # final relaxed snapshot for stage
├── scan_trj.xyz               # combined trajectory across all stages
└── scan.pdb                   # combined PDB when input is PDB
```

`result.json` lists per-stage status, target distances, and final
energies. Plot with `trj2fig.md`.
...
- Defaults: `import mlmm.core.defaults as d; print(d.BIAS_KW, d.BOND_KW, d.OUT_DIR_SCAN)`