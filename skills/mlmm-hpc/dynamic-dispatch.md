...
_worker.<PBS_JOBID>.sh  # generated by dispatcher, runs on each node
<TASK_LIST_FILE>        # one task per line (e.g. directory paths)
_state.<PBS_JOBID>.txt  # current task counter (auto-managed)
_lock.<PBS_JOBID>       # flock file (auto-managed)
```
...
JOB_ID="${PBS_JOBID:?PBS_JOBID is required for job-scoped dispatcher state}"
if [[ ! "${JOB_ID}" =~ ^[A-Za-z0-9._-]+$ ]]; then
    echo "Invalid PBS_JOBID: ${JOB_ID}" >&2
    exit 2
fi
STATE_FILE="${ROOT_DIR}/_state.${JOB_ID}.txt"
LOCK_FILE="${ROOT_DIR}/_lock.${JOB_ID}"
WORKER="${ROOT_DIR}/_worker.${JOB_ID}.sh"
