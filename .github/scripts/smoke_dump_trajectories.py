...
def _run_cli(args: list[str], timeout_sec: float | None = None) -> tuple[int, str]:
    """Run the CLI; return (returncode, combined stdout+stderr).

    A timeout still raises (a hung command is never acceptable); the caller
    decides whether a non-zero return code is tolerable for the case.
    """
