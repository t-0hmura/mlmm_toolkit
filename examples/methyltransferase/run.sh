#!/usr/bin/env bash
# Fur6 methyltransferase — methyl transfer reaction
...
mlmm opt -i complex_layered.pdb --parm parm/complex.parm7 -q -1 -m 1 --opt-mode grad --out-dir dump/opt1

# (ii) 1-D bond-length scan (atom indices 5411 and 5457, 0-based)
mlmm scan -i coord/1_opt_final_geometry.xyz --parm parm/complex.parm7 --model-pdb parm/ml_region.pdb -q -1 -m 1 --scan-lists "[(5411,5457,4.0)]" --max-step-size 0.05 --out-dir dump/scan
...
echo "Methyltransferase workflow complete."
