...
- **Optional backends**: `pip install "mlmm-toolkit[orb]"`, `"mlmm-toolkit[aimnet]"`. MACE requires `pip install --no-deps mace-torch` due to an e3nn version conflict with `fairchem-core`.
